...
This file provides theming guidance for Syncfusion Angular components. For framework-specific theming implementations, also refer to:
- **[greenfield-implementation.md](greenfield-implementation.md)** — Custom CSS design system with tokens and semantic classes
- **[tailwind-implementation.md](tailwind-implementation.md)** — Tailwind CSS utility-first approach with Tailwind3 Syncfusion theme
- **[material-implementation.md](material-implementation.md)** — Material Design 3 system-first approach with Material3 Syncfusion theme
- **[bootstrap-implementation.md](bootstrap-implementation.md)** — Bootstrap 5 component-first approach with Bootstrap5 Syncfusion theme
...
- Syncfusion pairing: Use Tailwind3 theme (components styled to match Tailwind's visual language)
...
- Syncfusion pairing: Use Bootstrap5 theme (components match Bootstrap's styling)
...
- Syncfusion pairing: Use Material3 theme (components follow Material Design 3 specs)
...
- Syncfusion pairing: Recommend Tailwind3 for syncfusion components
...
- Tailwind project + Tailwind3 Syncfusion theme = DataGrid uses Tailwind's color scale, spacing logic, responsive conventions
- Bootstrap project + Bootstrap5 Syncfusion theme = DataGrid inherits Bootstrap's component styling patterns
- Material project + Material3 Syncfusion theme = DataGrid follows Material Design elevation, spacing rules, typography system

**Example of misalignment (DON'T DO THIS):**
- Tailwind project + Bootstrap5 Syncfusion theme = DataGrid looks like Bootstrap; your custom components look like Tailwind; visual incoherence
- Material project + Bootstrap5 Syncfusion theme = Material components in Syncfusion, Bootstrap styling everywhere else; confusion

### Theme Selection Decision Tree

**If you detected Tailwind in Stage 2:**
→ Use **Tailwind3** Syncfusion theme
- Why: Components styled to Tailwind's visual language (clean, utility-based, minimal ornamentation)
- You'll extend with Tailwind utilities, not custom CSS

**If you detected Bootstrap in Stage 2:**
→ Use **Bootstrap5** Syncfusion theme
- Why: Components styled as Bootstrap components (component-first, opinionated defaults)
- You'll extend with Bootstrap utilities or component classes

**If you detected Material Design in Stage 2:**
→ Use **Material3** Syncfusion theme
- Why: Components follow Material Design System (elevation, spacing scale, semantic color system)
- You'll extend with Material tokens and Material-aligned CSS

**If Stage 2 detected no framework (greenfield project):**
→ Default to **Tailwind3** Syncfusion theme
- Why: Tailwind's utility-first philosophy is most flexible for new projects
- You're responsible for all design decisions; Tailwind gives you structure without being opinionated
...
- **Tailwind users:** You'll configure colors in `tailwind.config.js`. Tailwind's default scale includes semantic naming (primary, success, etc.). Follow that pattern.
- **Bootstrap users:** Bootstrap enforces semantic colors (primary, success, warning, danger). Respect Bootstrap's naming convention to avoid confusion.
- **Material users:** Material Design has specific color token naming (primary, secondary, tertiary, error) with fixed roles. Don't deviate.
...
- Tailwind users: Define dark mode in `tailwind.config.js` with strategy (class or prefers-color-scheme)
- Bootstrap users: Bootstrap has dark mode CSS variables; use them
- Material users: Material 3 specifies dark mode behavior; follow Material's rules
...
Refer to **Skill: syncfusion-angular-themes** → **Dark Mode Implementation** for:
...
- Recommendation: 4pt if you're designing custom components; 8pt if you want speed
...
- Tailwind: Provides a default typography scale; customize it in `tailwind.config.js`
- Bootstrap: Provides default sizes (sm, base, lg, etc.); extend or override
- Material: Specifies exact sizes for headline, body, label tiers; use Material's system
...
**Full-Height Container (standard web app):**
```css
.container {
  width: 100%;
...
- **Tailwind:** `tailwind.config.js` (extends or replaces defaults)
- **Bootstrap:** SCSS variables (often `_variables.scss` before importing Bootstrap)
- **Material:** Material Design tokens (following Material 3 spec)
- **Greenfield:** `styles.css` in your styles file

**Decision Point:** Are you using your framework's native token system or creating custom tokens?
- Native: Easier, matches framework ecosystem
- Custom: More control, but requires discipline to maintain
...
**Principle:** Your Syncfusion theme must be imported at app entry (before any components render).

This ensures:
- Components use the correct visual system
...
### 8.2 Custom CSS Coordination

**Non-Obvious Pattern:** Don't override Syncfusion component colors directly. Instead:

1. Define your color system in tokens
2. Syncfusion theme provides base styling
3. Custom CSS layers on top, using your tokens

**Example thinking:**
- ❌ Override DataGrid header color to `#ff0000`
- ✅ DataGrid inherits from Syncfusion theme, which respects your primary color token
...
Refer to **Skill: syncfusion-angular-themes** → **CSS Variables Customization** for:
...
- Tailwind 3.x setup with Syncfusion components
- `tailwind.config.js` configuration patterns
- Token architecture using Tailwind's color/spacing scale
- Pure utility-class component generation
- Responsive design with Tailwind breakpoints (sm, md, lg, xl)
- Syncfusion Tailwind3 theme integration
- Stage 6 validation checklist for Tailwind projects

**Key principle:** You'll define custom colors in `tailwind.config.js`, then extend Syncfusion's Tailwind3 theme using Tailwind utilities. No CSS variables needed.
...
**What this reference provides:**
- Bootstrap 5 setup with Syncfusion components
- Semantic color names (primary, secondary, success, danger, warning, info)
- 8pt spacing grid and responsive utilities
- 12-column grid system with mobile-first breakpoints (xs, sm, md, lg, xl, xxl)
- SCSS configuration and CSS variable setup
- Syncfusion Bootstrap5 theme integration
- Stage 6 validation checklist for Bootstrap projects

**Key principle:** Utility-first approach using Bootstrap semantic colors and spacing scale. You'll use utility classes for layout, spacing, and responsive design. Syncfusion Bootstrap5 theme handles component styling automatically.
...
- 4dp spacing grid (not 8pt); semantic color tokens (primary, secondary, tertiary, error, surface)
- Material typography roles (Display, Headline, Title, Body, Label)
- Elevation system 0-5 with tonal surfaces
- Canonical responsive layouts (Compact < 600dp, Medium 600-840dp, Expanded > 840dp)
- Material token configuration (CSS custom properties)
- Syncfusion Material3 theme integration
- Stage 6 validation checklist for Material Design projects

**Key principle:** Utility-first with Material system rules. All spacing from 4dp grid, typography from Material roles, colors from Material semantic tokens. Syncfusion Material3 theme handles component styling automatically.
...
**What this reference provides:**
- Custom design system architecture from scratch
- CSS custom properties (CSS variables) token system
- Tailwind3 Syncfusion theme as visual baseline
- Spacing scale design (4pt or 8pt grid, your choice)
- Typography scale with modular ratio
- Color system in OKLCH color space (perceptually uniform)
- Custom responsive breakpoint strategy
- Dark mode implementation strategy
- Syncfusion Tailwind3 theme integration
- Stage 6 validation checklist for greenfield projects

**Key principle:** You own all decisions. Recommended: use Tailwind3 Syncfusion theme as a visual foundation, then layer custom CSS variables and utilities. Maximum flexibility, maximum responsibility.
...
- ✅ Implementation approach locked:
  - **Tailwind:** Pure Tailwind utility classes (NOT CSS variables)
  - **Bootstrap:** Bootstrap utilities or component classes
  - **Material:** Material Design tokens + custom CSS
...
- **Framework setup** with correct Syncfusion theme imports
- **Token definitions** in framework-appropriate format (tailwind.config.js, SCSS variables, Material tokens)
- **Base component styles** following your accessibility standards
- **Responsive utilities** aligned to your breakpoint strategy
- **Syncfusion component integration** that respects your token system
...
### For All Projects:
- ✅ Syncfusion imports match locked theme (Tailwind3/Bootstrap5/Material3)
- ✅ WCAG 2.1 AA accessibility (contrast, focus states, reduced-motion)
- ✅ Responsive design verified on breakpoints (320px, 768px, 1024px, 1280px)
- ✅ TypeScript compilation without errors
- ✅ Build optimization (CSS purging, code splitting)

**Output:** Production-ready code aligned with Stage 4 design decisions
